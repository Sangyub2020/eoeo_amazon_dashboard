...
async function fetchAmazonOrders(
  ...
  // SP_API_BASE_URL은 함수 내부에서 동적으로 설정됨
  const baseUrl = baseUrl || SP_API_BASE_URL_DEFAULT;
  const endpoint = `${baseUrl}/orders/v0/orders`;
...
async function fetchOrderItems(
  ...
  const endpoint = `${SP_API_BASE_URL}/orders/v0/orders/${orderId}/orderItems`;
  const url = new URL(endpoint);
...
async function fetchOrderMetrics(
  ...
  const apiBaseUrl = baseUrl || SP_API_BASE_URL;
...
async function createReport(
  ...
  const endpoint = `${SP_API_BASE_URL}/reports/2021-06-30/reports`;
...
async function getReport(
  ...
  const endpoint = `${SP_API_BASE_URL}/reports/2021-06-30/reports/${reportId}`;
...
async function getReportDocument(
  ...
  const endpoint = `${SP_API_BASE_URL}/reports/2021-06-30/documents/${reportDocumentId}`;
...
async function fetchRefundsFromFinancialEvents(
  ...
  const endpoint = `${SP_API_BASE_URL}/finances/v0/financialEvents`;
  const url = new URL(endpoint);
...
async function fetchFBAInventory(
  ...
  const endpoint = `${SP_API_BASE_URL}/fba/inventory/v1/summaries`;
  const url = new URL(endpoint);
...
Deno.serve(async (req: Request) => {
  ...
  // OPTIONS 요청 처리 (가장 먼저 처리하여 CORS preflight 통과)
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      ...
      return new Response(
        JSON.stringify({
          ...
          note: "이 변수들은 Supabase Dashboard → Edge Functions → Settings → Secrets에서 설정해야 합니다.",
        }),
        {
          status: 500,
